...
namespace MedieticaWebApiService.Models
{
	public class ImgMarchiDb
	{
		...
		public static bool Search(ref OdbcCommand cmd, int codDit, int codice, short formato, ref ImgMarchiDb img, bool partial = false, bool writeLock = false)
		{
			...
			string sql;
			if (partial)
				sql = DbUtils.QueryAdapt("SELECT img_dit, img_codice, img_formato, img_tipo, img_bytes_size, img_created_at, img_last_update FROM imgmarchi WHERE img_dit = ? AND img_codice = ? AND img_formato = ?", 1);
			else 
				sql = DbUtils.QueryAdapt("SELECT * FROM imgmarchi WHERE img_dit = ? AND img_codice = ? AND img_formato = ?", 1);
		...
		public static void Write(ref OdbcCommand cmd, DbMessage msg, ref ImgMarchiDb img, ref object obj, bool partial = true)
		{
			DbUtils.Trim(ref img);

			if (msg == DbMessage.DB_UPDATE || msg == DbMessage.DB_DELETE || msg == DbMessage.DB_CLEAR || msg == DbMessage.DB_REWRITE)
			{
				var old = new ImgMarchiDb();
			...
			switch (msg)
			{
				case DbMessage.DB_INSERT:
					{
						do
						{
							try
							{
								...
								cmd.CommandText = DbUtils.SqlCommand(ref cmd, DbMessage.DB_INSERT, ref img, "imgmarchi");
								cmd.ExecuteNonQuery();

								//
								// Scriviamo la miniatura
								// 
								cmd.CommandText = DbUtils.QueryAdapt("DELETE FROM imgmarchi WHERE img_dit = ? AND img_codice = ? AND img_formato = ?");
								cmd.Parameters.Clear();
								cmd.Parameters.Add("coddit", OdbcType.Int).Value = img.img_dit;
								cmd.Parameters.Add("codice", OdbcType.Int).Value = img.img_codice;
								...
								cmd.CommandText = DbUtils.SqlCommand(ref cmd, DbMessage.DB_INSERT, ref thu, "imgmarchi");
								cmd.ExecuteNonQuery();
							}
				...
				{
					cmd.CommandText = DbUtils.QueryAdapt("DELETE FROM imgmarchi WHERE img_dit = ? AND img_codice = ? AND img_formato IN (?, ?)");
					cmd.Parameters.Clear();
					cmd.Parameters.Add("coddit", OdbcType.Int).Value = img.img_dit;
					cmd.Parameters.Add("codice", OdbcType.Int).Value = img.img_codice;
					...
					var sql = "SELECT img_formato FROM imgmarchi WHERE img_dit = ? AND img_codice = ? ORDER BY img_formato FOR UPDATE NOWAIT";
					...
					short last = 0;
					sql = DbUtils.QueryAdapt("UPDATE imgmarchi SET img_formato = ?, img_last_update = Now() WHERE img_dit = ? AND img_codice = ? AND img_formato = ?");
		...
		public static void Swap(ref OdbcCommand cmd, DbMessage msg, ref ImagesSwap swa, ref IList<ImagesSwapList> arr, bool parial = false)
		{
			...
			//
			sql = DbUtils.QueryAdapt($"UPDATE imgmarchi SET img_formato = ? WHERE img_dit = {swa.img_dit} AND img_codice = {swa.img_codice} AND img_formato = ?");
			foreach (var swp in arr)
			{
			...
			//
			short last = 0;
			sql = DbUtils.QueryAdapt($"UPDATE imgmarchi SET img_formato = ?, img_last_update = Now() WHERE img_dit = {swa.img_dit} AND img_codice = {swa.img_codice} AND img_formato = ?");
			foreach (var swp in arr)
			{
				var old_img = swp.img_formato % 2 == 0 ? swp.img_formato : swp.img_formato - 1;
				var old_thu = swp.img_formato % 2 != 0 ? swp.img_formato : swp.img_formato + 1;
		...
		public static void Reload(ref OdbcCommand cmd, ref ImgMarchiDb img, bool partial)
		{
