...
namespace MedieticaWebApiService.Models
{
	public class ImgModelliDb
	{
		...
		public static bool Search(ref OdbcCommand cmd, int codDit, int codice, short formato, ref ImgModelliDb img, bool partial = false, bool writeLock = false)
		{
			...
			string sql;
			if (partial)
				sql = DbUtils.QueryAdapt("SELECT img_dit, img_codice, img_formato, img_tipo, img_bytes_size, img_created_at, img_last_update FROM imgmodelli WHERE img_dit = ? AND img_codice = ? AND img_formato = ?", 1);
			else 
				sql = DbUtils.QueryAdapt("SELECT * FROM imgmodelli WHERE img_dit = ? AND img_codice = ? AND img_formato = ?", 1);
		...
		public static void Write(ref OdbcCommand cmd, DbMessage msg, ref ImgModelliDb img, ref object obj, bool partial = true)
		{
			DbUtils.Trim(ref img);

			if (msg == DbMessage.DB_UPDATE || msg == DbMessage.DB_DELETE || msg == DbMessage.DB_CLEAR || msg == DbMessage.DB_REWRITE)
			{
				var old = new ImgModelliDb();
			...
			if (msg == DbMessage.DB_INSERT || msg == DbMessage.DB_UPDATE)
			{
				...
				ModelliDb mod = null;
				if (!ModelliDb.Search(ref cmd, img.img_dit, img.img_codice, ref mod)) throw new MCException(MCException.ModelloMsg, MCException.ModelloErr);
			}
			switch (msg)
			{
				case DbMessage.DB_INSERT:
					{
						do
						{
							...
							{
								//
								// Scriviamo l'immagine ridimensionata
								// 
								...
								if (image.Width > MaxSize || image.Height > MaxSize)
								{
									...
									if (image.Width > image.Height)
										mul = MaxSize / (double)image.Width;
									else
										mul = MaxSize / (double)image.Height;
								...
								var thu = new ImgModelliDb();
								thu.img_dit = img.img_dit;
								thu.img_codice = img.img_codice;
								thu.img_tipo = img.img_tipo;
								thu.img_formato = (short)(img.img_formato);
								thu.img_bytes_size = data.Length;
								thu.img_data = Convert.ToBase64String(data);

								cmd.CommandText = DbUtils.SqlCommand(ref cmd, DbMessage.DB_INSERT, ref thu, "imgmodelli");
								cmd.ExecuteNonQuery();

								//
								// Scriviamo la miniatura
								// 
								cmd.CommandText = DbUtils.QueryAdapt("DELETE FROM imgmodelli WHERE img_dit = ? AND img_codice = ? AND img_formato = ?");
								cmd.Parameters.Clear();
								...
								image = (Bitmap)ByteArrayToImage(Convert.FromBase64String(img.img_data));
								...
								data = ImageToByteArray(image, img.img_tipo != 15 ? ImageFormat.Jpeg : ImageFormat.Png);
								thu = new ImgModelliDb();
								thu.img_dit = img.img_dit;
								thu.img_codice = img.img_codice;
								thu.img_tipo = img.img_tipo;
				...
				{
					cmd.CommandText = DbUtils.QueryAdapt("DELETE FROM imgmodelli WHERE img_dit = ? AND img_codice = ? AND img_formato IN (?, ?)");
					cmd.Parameters.Clear();
					cmd.Parameters.Add("coddit", OdbcType.Int).Value = img.img_dit;
					cmd.Parameters.Add("codice", OdbcType.Int).Value = img.img_codice;
					...
					var sql = "SELECT img_formato FROM imgmodelli WHERE img_dit = ? AND img_codice = ? ORDER BY img_formato FOR UPDATE NOWAIT";
					...
					short last = 0;
					sql = DbUtils.QueryAdapt("UPDATE imgmodelli SET img_formato = ?, img_last_update = Now() WHERE img_dit = ? AND img_codice = ? AND img_formato = ?");
		...
		public static void Swap(ref OdbcCommand cmd, DbMessage msg, ref ImagesSwap swa, ref IList<ImagesSwapList> arr, bool parial = false)
		{
			...
			//
			var first = true;
			var sql = "SELECT * FROM imgmodelli";
			...
			//
			sql = DbUtils.QueryAdapt($"UPDATE imgmodelli SET img_formato = ? WHERE img_dit = {swa.img_dit} AND img_codice = {swa.img_codice} AND img_formato = ?");
			foreach (var swp in arr)
			{
			...
			//
			short last = 0;
			sql = DbUtils.QueryAdapt($"UPDATE imgmodelli SET img_formato = ?, img_last_update = Now() WHERE img_dit = {swa.img_dit} AND img_codice = {swa.img_codice} AND img_formato = ?");
			foreach (var swp in arr)
			{
				var old_img = swp.img_formato % 2 == 0 ? swp.img_formato : swp.img_formato - 1;
				var old_thu = swp.img_formato % 2 != 0 ? swp.img_formato : swp.img_formato + 1;
		...
		public static void Reload(ref OdbcCommand cmd, ref ImgModelliDb img, bool partial)
		{
