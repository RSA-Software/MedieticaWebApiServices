...
{
	[EnableCors("*", "*", "*")]
	public class ImgDitteController : ApiController
	{
		[HttpGet]
		[Route("api/imgditte/getthumbnails/{ditta}/{codice}")]
		public DefaultJson<ImgDitteDb> GetThumbnails(int ditta, int codice)
		{
			var json = new DefaultJson<ImgDitteDb>();
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					while (reader.Read())
					{
						...
						if (json.Data == null) json.Data = new List<ImgDitteDb>();
						json.Data.Add(img);
						json.RecordsTotal++;
					}
					reader.Close();
		...
		[HttpGet]
		[Route("api/imgditte/get/{ditta}/{codice}/{formato}")]
		public DefaultJson<ImgDitteDb> Get(int ditta, int codice, short formato)
		{
			var json = new DefaultJson<ImgDitteDb>();
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					var img = new ImgDitteDb();
					if (ImgDitteDb.Search(ref cmd, ditta, codice, formato, ref img))
					{
						if (json.Data == null) json.Data = new List<ImgDitteDb>();
						json.Data.Add(img);
						json.RecordsTotal++;
		...
		[HttpGet]
		[Route("api/imgditte/getimage/{ditta}/{codice}/{formato}")]
		public DefaultJson<ImagesJson> GetImage(int ditta, int codice, short formato)
		{
			...
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					if (ImgDitteDb.Search(ref cmd, ditta, codice, formato, ref img))
					{
						var image = ImgDitteDb.ByteArrayToImage(Convert.FromBase64String(img.img_data));
						...
						if ((bmp_w > max_image_size) || (bmp_h > max_image_size))
						{
							...
							var bitmap = ImgDitteDb.ResizeImage(image, bmp_w, bmp_h);
							img.img_data = Convert.ToBase64String(ImgDitteDb.ImageToByteArray(bitmap, image.RawFormat));
						}
		...
		[HttpPost]
		[Route("api/imgditte/post/{ditta}/{codice}")]
		public async Task<DefaultJson<ImgDitteDb>> Post(int ditta, int codice)
		{
			var json = new DefaultJson<ImgDitteDb>();
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					foreach (MultipartFileData file in provider.FileData)
					{
						...
						object obj = null;
						DbUtils.SqlWrite(ref cmd, ImgDitteDb.Write, DbMessage.DB_INSERT, ref img, ref obj, true);

						if (!ImgDitteDb.Search(ref cmd, img.img_dit, img.img_codice, (short)(img.img_formato + 1), ref img)) throw new MCException(MCException.NotFoundMsg, MCException.NotFoundErr);
		...
		[HttpPut]
		[Route("api/imgditte/swap/{ditta}/{codice}")]
		public DefaultJson<ImgDitteDb> Swap(int ditta, int codice, [FromBody] DefaultJson<ImagesSwapList> value)
		{
			DefaultJson<ImgDitteDb> json;
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					var arr = value.Data;
					DbUtils.SqlWrite(ref cmd, ImgDitteDb.Swap, DbMessage.DB_INSERT, ref swa, ref arr);
		...
		[HttpDelete]
		[Route("api/imgditte/delete/{ditta}/{codice}/{formato}")]
		public DefaultJson<ImgDitteDb> Delete(int ditta, int codice, short formato)
		{
			...
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					var img = new ImgDitteDb();
					if (!ImgDitteDb.Search(ref cmd, ditta, codice, formato, ref img)) throw new HttpResponseException(Request.CreateResponse(HttpStatusCode.NotFound, "Risorsa non trovata"));

					object objx = null;
					DbUtils.SqlWrite(ref cmd, ImgDitteDb.Write, DbMessage.DB_DELETE, ref img, ref objx);
