...
{
	[EnableCors("*", "*", "*")]
	public class ImgGiornaleLavoriController : ApiController
	{
		[HttpGet]
		[Route("api/imggiornalelavori/getthumbnails/{ditta}/{codice}")]
		public DefaultJson<ImgGiornaliLavoriDb> GetThumbnails(int ditta, int codice)
		{
			var json = new DefaultJson<ImgGiornaliLavoriDb>();
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					cmd.CommandText = "SELECT * FROM giornaleimg WHERE img_dit = ? AND img_codice = ? AND Mod(img_formato, 2) <> 0 ORDER BY img_formato, img_codice";
					...
					while (reader.Read())
					{
						...
						if (json.Data == null) json.Data = new List<ImgGiornaliLavoriDb>();
						json.Data.Add(img);
						json.RecordsTotal++;
					}
					reader.Close();
		...
		[HttpGet]
		[Route("api/imggiornalelavori/get/{ditta}/{codice}/{formato}")]
		public DefaultJson<ImgGiornaliLavoriDb> Get(int ditta, int codice, short formato)
		{
			var json = new DefaultJson<ImgGiornaliLavoriDb>();
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					var img = new ImgGiornaliLavoriDb();
					if (ImgGiornaliLavoriDb.Search(ref cmd, ditta, codice, formato, ref img))
					{
						if (json.Data == null) json.Data = new List<ImgGiornaliLavoriDb>();
						json.Data.Add(img);
						json.RecordsTotal++;
		...
		[HttpGet]
		[Route("api/imggiornalelavori/getimage/{ditta}/{codice}/{formato}")]
		public DefaultJson<ImagesJson> GetImage(int ditta, int codice, short formato)
		{
			...
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					if (ImgGiornaliLavoriDb.Search(ref cmd, ditta, codice, formato, ref img))
					{
						var image = ImgGiornaliLavoriDb.ByteArrayToImage(Convert.FromBase64String(img.img_data));
						...
						if ((bmp_w > max_image_size) || (bmp_h > max_image_size))
						{
							...
							var bitmap = ImgGiornaliLavoriDb.ResizeImage(image, bmp_w, bmp_h);
							img.img_data = Convert.ToBase64String(ImgGiornaliLavoriDb.ImageToByteArray(bitmap, image.RawFormat));
						}
		...
		[HttpPost]
		[Route("api/imggiornalelavori/post/{ditta}/{codice}")]
		public async Task<DefaultJson<ImgGiornaliLavoriDb>> Post(int ditta, int codice)
		{
			var json = new DefaultJson<ImgGiornaliLavoriDb>();
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					cmd.CommandText = DbUtils.QueryAdapt("SELECT COALESCE(CAST(ABS(MAX(img_formato)) AS INT), 0) AS codice FROM giornaleimg WHERE img_dit = ? AND img_codice = ? AND Mod(img_formato, 2) = 0");
					cmd.Parameters.Clear();
					cmd.Parameters.Add("ditta", OdbcType.Int).Value = ditta;
					cmd.Parameters.Add("codice", OdbcType.Int).Value = codice;
					...
					foreach (MultipartFileData file in provider.FileData)
					{
						...
						object obj = null;
						DbUtils.SqlWrite(ref cmd, ImgGiornaliLavoriDb.Write, DbMessage.DB_INSERT, ref img, ref obj, true);

						if (!ImgGiornaliLavoriDb.Search(ref cmd, img.img_dit, img.img_codice, (short)(img.img_formato + 1), ref img)) throw new MCException(MCException.NotFoundMsg, MCException.NotFoundErr);
		...
		[HttpPut]
		[Route("api/imggiornalelavori/swap/{ditta}/{codice}")]
		public DefaultJson<ImgGiornaliLavoriDb> Swap(int ditta, int codice, [FromBody] DefaultJson<ImagesSwapList> value)
		{
			DefaultJson<ImgGiornaliLavoriDb> json;
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					var arr = value.Data;
					DbUtils.SqlWrite(ref cmd, ImgGiornaliLavoriDb.Swap, DbMessage.DB_INSERT, ref swa, ref arr);
		...
		[HttpDelete]
		[Route("api/imggiornalelavori/delete/{ditta}/{codice}/{formato}")]
		public DefaultJson<ImgGiornaliLavoriDb> Delete(int ditta, int codice, short formato)
		{
			DefaultJson<ImgGiornaliLavoriDb> json;
			try
			{
				using (var connection = new OdbcConnection(DbUtils.GetConnectionString()))
				{
					...
					var img = new ImgGiornaliLavoriDb();
					if (!ImgGiornaliLavoriDb.Search(ref cmd, ditta, codice, formato, ref img)) throw new HttpResponseException(Request.CreateResponse(HttpStatusCode.NotFound, "Risorsa non trovata"));

					object objx = null;
					DbUtils.SqlWrite(ref cmd, ImgGiornaliLavoriDb.Write, DbMessage.DB_DELETE, ref img, ref objx);
					json = GetThumbnails(ditta, codice);
